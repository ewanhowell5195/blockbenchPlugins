(() => {
  ...
  let originalJEMCodec, scaleProperty

  function loadOptiFineEntityExtensions() {
    scaleProperty = new Property(Group, "number", "cem_scale", {
      default: 1,
      exposed: false
    })
    ...
    const originalLocatorClick = BarItems.add_locator.click
    const customLocatorClick = () => {
      const objs = []
      Undo.initEdit({ elements: objs, outliner: true })
      const attachment = new Locator({ name: "attachment_point" }).addTo(Group.selected || Group.all[0]).init()
      attachment.select().createUniqueName()
      objs.push(attachment)
      Undo.finishEdit("Add attachment point")
      Vue.nextTick(() => {
    ...
    Formats.optifine_entity.locators = true

    Formats.optifine_entity.onActivation = () => {
      Locator.prototype.title = "Attachment Point"
      Locator.prototype.menu = customLocatorMenu
      Locator.prototype.icon = "attachment"
      BarItems.add_locator.name = "Add Attachment Point"
      BarItems.add_locator.description = "Add an attachment point where items and blocks can attach to the model"
      BarItems.add_locator.click = customLocatorClick
      BarItems.add_locator.icon = "attachment"
      updateAttachmentIcon()
    }

    Formats.optifine_entity.onDeactivation = () => {
      Locator.prototype.title = tl("data.locator")
      Locator.prototype.menu = originalLocatorMenu
      Locator.prototype.icon = "fa-anchor"
      BarItems.add_locator.name = tl("action.add_locator")
      BarItems.add_locator.description = tl("action.add_locator.desc")
      BarItems.add_locator.click = originalLocatorClick
      BarItems.add_locator.icon = "fa-anchor"
    }
    ...
    originalJEMCodec = {
      compile: Codecs.optifine_entity.compile,
      parse: Codecs.optifine_entity.parse
    }

    Codecs.optifine_entity.compile = function(options = {}) {
      const entitymodel = {}
      ...
      if (Project.shadow_size != 1) {
        entitymodel.shadowSize = Project.shadow_size
      }
      entitymodel.models = []
      ...
      Outliner.root.forEach(g => {
        ...
        if (!isNaN(g.cem_scale) && g.cem_scale !== 1) {
          bone.scale = g.cem_scale
        }

        function populate(p_model, group, root) {
          ...
          group.children.forEach(obj => {
            ...
            if (obj.type === "cube") {
              ...
              if (obj.mirror_uv !== group.mirror_uv && has_different_mirrored_children) {
                ...
                p_model.boxes ??= []
                if (obj.mirror_uv && obj.mirror_uv !== group.mirror_uv) {
                  p_model.mirrorTexture = "u"
                }
            ...
            } else if (obj.type === "locator") {
              p_model.attachments ??= {}
              p_model.attachments[obj.name] = [
                (obj.position[0] - group.origin[0]) / 16 * -1,
                (obj.position[1] - group.origin[1]) / 16 * -1,
                (obj.position[2] - group.origin[2]) / 16
              ]
            }
          })
        }
        populate(bone, g, true)
    ...
    Codecs.optifine_entity.parse = function(model, path) {
      ...
      if (model.models) {
        model.models.forEach(function(b) {
          ...
          const group = new Group({
            ...
            origin: b.translate,
            rotation: b.rotate,
            mirror_uv: !!b.mirrorTexture?.includes("u"),
          ...
          function readContent(submodel, p_group, root) {
            if (submodel.boxes && submodel.boxes.length) {
              submodel.boxes.forEach(box => {
                ...
                if (!box.textureOffset && (box.uvNorth || box.uvEast || box.uvSouth || box.uvWest || box.uvUp || box.uvDown)) {
                  base_cube.extend({
                    box_uv: false,
                    faces: {
            ...
            if (submodel.submodels && submodel.submodels.length) {
              submodel.submodels.forEach(subsub => {
                ...
                const group = new Group({
                  ...
                  origin: subsub.translate || (root ? undefined : submodel.translate),
            ...
            if (submodel.attachments) {
              for (const [name, translate] of Object.entries(submodel.attachments)) {
                const attachment = new Locator({ name })
                attachment.position[0] = (translate[0] * 16 * -1) + p_group.origin[0]
                attachment.position[1] = (translate[1] * 16 * -1) + p_group.origin[1]
                attachment.position[2] = (translate[2] * 16) + p_group.origin[2]
                attachment.addTo(p_group).init()
              }
            }
          }
          group.init().addTo()
          readContent(b, group, true)
        })
      }
  ...
  function unloadOptiFineEntityExtensions() {
    scaleProperty.delete()
    Formats.optifine_entity.locators = false
    Formats.optifine_entity.onActivation = undefined
    Formats.optifine_entity.onDeactivation()
    Formats.optifine_entity.onDeactivation = undefined
    Codecs.optifine_entity.compile = originalJEMCodec.compile
    Codecs.optifine_entity.parse = originalJEMCodec.parse
  }

  const attachmentImage = new THREE.TextureLoader().load("data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAACAAAAAgAQMAAABJtOi3AAAABlBMVEX///////9VfPVsAAAAAXRSTlMAQObYZgAAAFBJREFUCB2FwDEVgCAUQNF3Drs0wBoOhILNzZ+ATERiZHs20MsPnSRdZN2cQaVC4YKD+3HQdNB10CeFPql03TRd3I+TC6ACnAFkDZIGaPDpBYSrJ4N0uEaGAAAAAElFTkSuQmCC")

  function updateAttachmentIcon() {
    setTimeout(() => scene.traverse(e => {
      if (e.type === "locator" && e.sprite) {
        e.sprite.material.map = attachmentImage
      }
    }), 0)
  }
